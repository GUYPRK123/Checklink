...
3. **`python app.py` ผูกกับ `127.0.0.1`** (`app.py:109`) — เข้าจากภายนอกไม่ได้ ต้องใช้ `waitress-serve --host=0.0.0.0` สำหรับ production
...
```
POST /api/check    body = {"url": "..."}    -> ผลการวิเคราะห์ (JSON)
```
...
│  ├─ check.py            # endpoint การเช็กลิงก์
│  ├─ models.py           # ตารางฐานข้อมูล
│  ├─ analyzer/           # หัวใจการวิเคราะห์ (cascade 4 ชั้น)
│  └─ requirements.txt
