...
**หลักฐานฝั่งปลอดภัย (trust evidence)** — ระบบยืนยันว่า "เป็นเว็บจริง" ได้ 3 ทาง:
โดเมนตรงลิสต์แบรนด์ / บัญชี สกมช. บอกว่าปลอดภัย / หรือ **ไม่มีสัญญาณเสี่ยงเลยแม้แต่
ตัวเดียว + มีหลักฐานยืนยันตัวตนพอ** โดยหลักฐานมาจากสองแหล่งที่ไม่ต้องแตะเว็บปลายทางเลย:

- **นามสกุลที่ต้องพิสูจน์ตัวตนกับนายทะเบียนก่อนจด** — `.go.th` `.ac.th` `.mi.th` `.gov.uk`
  สงวนให้ราชการ/สถานศึกษา มิจฉาชีพจดไม่ได้ จึงยืนยันได้ด้วยตัวเอง ส่วน `.co.th`
  `.or.th` `.net.th` `.co.jp` ต้องใช้เอกสารนิติบุคคล — ยืนยันได้แค่ "มีตัวตนตามกฎหมาย"
  จึงนับเป็นหลักฐานประกอบ ต้องมีอย่างอื่นมาเสริมถึงจะเขียว
- **ความนิยมของโดเมน** (`backend/analyzer/popularity.py`) — เทียบกับรายการอันดับ Tranco
  ที่เก็บไว้ **เป็นไฟล์ในเครื่อง** จึงไม่มีแพ็กเก็ตวิ่งไปหาเว็บที่กำลังตรวจ (เจ้าของเว็บ
  ไม่รู้ตัวว่าถูกตรวจ) และเร็วพอจะใช้ในชั้น 2 ที่ผู้ไม่ล็อกอินก็ได้ใช้

> ⚠️ **"อันดับสูง" ≠ "ปลอดภัย"** — โดเมนของบริการฝากเว็บฟรีติดอันดับต้น ๆ ทั้งนั้น
> (`amazonaws.com` อันดับ 7 ของโลก, `github.io` อันดับ 121) วัดกับ `testset_100.json`
> พบว่าลิงก์ฟิชชิ่งที่ยังทำงานอยู่จริง 22 จาก 50 ลิงก์อยู่ในรายการอันดับ **และทุกอัน
> เป็นโดเมนของแพลตฟอร์ม ไม่ใช่ของคนหลอกเอง** โดเมนใน `USER_CONTENT_DOMAINS` และ
> `SHORTENERS` จึงถูกตัดสิทธิ์รับหลักฐานฝั่งปลอดภัยทั้งหมด

หลักฐานฝั่งนี้ **มี 0 คะแนนเสมอ** จึงไม่มีทางลดระดับความเสี่ยงของลิงก์ที่มีสัญญาณอันตราย
— มันใช้ยกจาก "เหลืองเพราะไม่รู้จัก" เป็นเขียวเท่านั้น (ดูเหตุผลเต็มใน `analyzer/config.py`)

อัปเดตรายการอันดับ (ควรทำปีละครั้งสองครั้ง): `cd backend && ./.venv/bin/python tools/update_popularity.py`
...
แปลว่า QR ถูกแก้ไข (แดง) ถ้าไม่มี CRC มาให้เลยคือ "ยืนยันไม่ได้" (เหลือง) และชื่อผู้รับเงิน
ที่มีอักขระมองไม่เห็นแทรกอยู่ก็ถูกจับด้วย เพราะชื่อคือสิ่งเดียวที่ผู้ใช้ใช้ยืนยันก่อนกดโอน
รองรับ Wi-Fi / เบอร์โทร / SMS / อีเมล / นามบัตร / พิกัด รวมถึง QR ที่เปิดแอปโดยตรง
(`line://`, `market://`, `intent://` ฯลฯ) และ QR โอนคริปโต (`bitcoin:`, `ethereum:` ฯลฯ)
QR ที่เป็นลิงก์ถูกส่งเข้า cascade 4 ชั้นเดียวกับโหมดลิงก์ทุกประการ

ความยาวใน EMVCo นับเป็น **ไบต์** และ CRC คิดจากไบต์เช่นกัน ตัวถอดจึงเดินบน UTF-8 ไม่ใช่ตัวอักษร
ไม่งั้น QR ของร้านที่ตั้งชื่อเป็นภาษาไทยจะถูกตัดสินว่า "ถูกแก้ไข" ทั้งที่ไม่ได้ถูกแก้
...
6. **เพิ่มโดเมนเข้า `USER_CONTENT_DOMAINS` ทุกครั้งที่เจอบริการฝากเว็บใหม่** — ลิสต์นี้คือ
...
| `POPULARITY_LIST` | - | `backend/analyzer/data/popular_domains.txt` (ไม่มีไฟล์ = ปิดการเทียบความนิยมเงียบ ๆ) |
...
|---|---|---|
| `POST /api/check` | ทุกคน | body = `{"url": "..."}` — ผู้ไม่ล็อกอินติดโควตา/วัน (ตอบ 429 เมื่อครบ) · ลิงก์ยาวเกิน 2,048 ตัวอักษรตอบ 400 |
| `POST /api/check/qr` | ทุกคน | body = `{"payload": "<เนื้อหาที่ถอดจาก QR>"}` — โควตาเดียวกับ `/api/check` · payload ยาวเกิน 4,096 ตัวอักษรตอบ 400 (QR จริงเก็บได้ราว 2,953 ไบต์) |
