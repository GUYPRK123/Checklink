...
function scanTimelineHTML(res) {
  ...
  // เหตุผลที่ชั้นลึกถูกข้าม — มีสองตระกูลที่ห้ามปนกัน:
  //   skipped_reason = ข้ามเพราะฝั่งระบบ (คิวตรวจเชิงลึกเต็ม) -> ลองใหม่อีกครู่ก็ได้
  //   locked_reason  = ข้ามเพราะสิทธิ์ของผู้ใช้ไม่ถึง         -> ต้องอัพเกรด/ล็อกอิน
  // เช็ก skipped_reason ก่อนเสมอ ไม่งั้นสมาชิกพรีเมียมจะเห็นข้อความว่า "ต้องอัพเกรด"
  // ทั้งที่จ่ายเงินแล้ว เพียงเพราะระบบบังเอิญแน่นตอนนั้น
  const skipDetail = dc.skipped_reason === "server_busy"
    ? "ข้าม — ระบบกำลังตรวจเชิงลึกเต็มคิว ลองตรวจลิงก์นี้อีกครั้งในอีกสักครู่"
    : ({
